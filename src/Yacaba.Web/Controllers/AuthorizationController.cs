...
public class AuthorizationController : Controller {
    ...
    public async Task<IActionResult> Authorize() {
        ...
        Object application = await _applicationManager.FindByClientIdAsync(request.ClientId ?? throw new InvalidDataException("ClientId is required")) ??
        ...
        switch (await _applicationManager.GetConsentTypeAsync(application)) {
            ...
            case ConsentTypes.Explicit when authorizations.Count is not 0 && !request.HasPromptValue(PromptValues.Consent):
                ...
                authorization ??= await _authorizationManager.CreateAsync(
                    identity: identity,
                    subject: await _userManager.GetUserIdAsync(user),
                    client: await _applicationManager.GetIdAsync(application) ?? throw new InvalidDataException("Client is required"),
                    type: AuthorizationTypes.Permanent,
            ...
            default:
                return View(new AuthorizeViewModel {
                    ApplicationName = await _applicationManager.GetLocalizedDisplayNameAsync(application) ?? throw new InvalidDataException("ApplicationName is required"),
                    Scope = request.Scope ?? throw new InvalidDataException("Scope is required")
                });
        }
    }
    ...
    public async Task<IActionResult> Exchange() {
        ...
        if (request.IsAuthorizationCodeGrantType() || request.IsRefreshTokenGrantType()) {
            ...
            ApplicationUser? user = await _userManager.FindByIdAsync(result.Principal?.GetClaim(Claims.Subject) ?? throw new InvalidDataException("Subject is required"));
            if (user is null) {
                return Forbid(
                    authenticationSchemes: OpenIddictServerAspNetCoreDefaults.AuthenticationScheme,
                    properties: new AuthenticationProperties(new Dictionary<String, String?> {
